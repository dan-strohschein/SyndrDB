...
// Run is the main entry point for the SyndrDB server.
// It is exported so that enterprise builds can import and call it from
// their own main package after registering extensions.
func Run() {
	...
	flag.BoolVar(&args.ExportRealtimeMetrics, "export-RT-metrics", args.ExportRealtimeMetrics, "Export Real Time Metrics for Monitoring")
	flag.IntVar(&args.QueryTimeoutSeconds, "query-timeout", args.QueryTimeoutSeconds, "Default query execution timeout in seconds")
	flag.IntVar(&args.AdminQueryTimeoutSeconds, "admin-query-timeout", args.AdminQueryTimeoutSeconds, "Admin query execution timeout in seconds")
	flag.IntVar(&args.QueryMaxMemoryMB, "query-max-memory", args.QueryMaxMemoryMB, "Maximum memory per query in MB")
	flag.IntVar(&args.AdminQueryMaxMemoryMB, "admin-query-max-memory", args.AdminQueryMaxMemoryMB, "Maximum memory per admin query in MB")
	...
	if args.AuthEnabled {
		...
		if userCount == 0 && len(args.DefaultUsers) == 0 {
			log.Printf("WARNING: Authentication is enabled but no users exist. Create users using 'ADD USER' command or configure default_users in config file.")
		}
	}
	...
	if reg := extension.GetRegistry(); reg != nil {
		if err := reg.NotifyServerStart(context.Background(), extCtx); err != nil {
			log.Fatalf("Enterprise extension startup failed: %v", err)
		}
	}
	...
	if args.WhereSIMDEnabled {
		...
		log.Printf("WARNING: WHERE clause SIMD optimization disabled - using scalar comparisons")
