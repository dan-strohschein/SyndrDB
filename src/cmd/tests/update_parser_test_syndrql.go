...
func TestUpdateParser_BasicUpdate(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			input:          `UPDATE DOCUMENTS IN BUNDLE "Authors" ("AuthorName" = "Dan Strohschein") WHERE "DocumentID" == "123";`,
			expectError:    false,
			expectedBundle: "Authors",
			expectedFields: map[string]interface{}{
				"AuthorName": "Dan Strohschein",
			},
		},
		{
	...
	}{
		{
			...
			checkWhere: func(t *testing.T, expr syndrQL.Expression) {
				...
				if binExpr.Operator != syndrQL.TOKEN_EQ {
					t.Errorf("Expected EQ operator, got %v", binExpr.Operator)
				}
			},
		},
		{
...
func TestUpdateParser_EdgeCases(t *testing.T) {
	tests := []struct {
		...
		// TODO: Add support for negative number literals in field assignments
		// Currently -50.25 is tokenized as MINUS + NUMBER, not as single negative number
		// {
...
// TestUpdateParser_QuotedFieldNamesWithAdapter tests the full pipeline including adapter
func TestUpdateParser_QuotedFieldNamesWithAdapter(t *testing.T) {
	// Test the exact query from the user that was failing
	input := `UPDATE DOCUMENTS IN BUNDLE "Authors" ("AuthorName" = "Dan Strohschein-666") WHERE "DocumentID" == "187320fc9a770e28_33";`

	parser, err := syndrQL.NewUpdateParser(input)
	if err != nil {
	...
	if stmt.Fields["AuthorName"] != "Dan Strohschein-666" {
		t.Errorf("Expected field AuthorName with value 'Dan Strohschein-666', got '%v'", stmt.Fields["AuthorName"])
	}
	...
	// Now test the adapter - this is where the original error occurred
	adapter := syndrQL.NewUpdateStatementAdapter(nil)
	docCmd, err := adapter.ToDocumentUpdateCommand(stmt)
	if err != nil {
		t.Fatalf("Failed to convert to DocumentUpdateCommand: %v", err)
	}

	if docCmd.BundleName != "Authors" {
		t.Errorf("Expected bundle 'Authors' in command, got '%s'", docCmd.BundleName)
	}

	if docCmd.WhereClause == "" {
		t.Error("WHERE clause is empty in DocumentUpdateCommand")
	}

	// Verify the WHERE clause contains the field name and value
	if !containsString(docCmd.WhereClause, "DocumentID") {
		t.Errorf("WHERE clause should contain 'DocumentID', got: %s", docCmd.WhereClause)
	}

	t.Logf("Successfully parsed and adapted query with quoted field names")
	t.Logf("WHERE clause: %s", docCmd.WhereClause)
}
