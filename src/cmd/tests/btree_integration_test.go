...
//
// NOTE: This test file is currently disabled because it requires a fully initialized
// BundleService which cannot be easily constructed in a unit test context.
// These tests need to either:
// 1. Be converted to integration tests with proper service setup
// 2. Use mocking/dependency injection
// 3. Be moved to the bundle package as internal tests
...
func createTestBundleService() *bundle.BundleService {
	// Cannot construct BundleService without full dependencies
	// Would need: bundleStore, databaseService, documentScanner, settings, etc.
	return nil
}
