...
func TestBloomFilterFalsePositiveRate(t *testing.T) {
	...
	bf := bloomfilter.NewBloomFilter(expectedItems, targetFPR)
...
func TestBloomFilterStats(t *testing.T) {
	expectedItems := 1000
	bf := bloomfilter.NewBloomFilter(expectedItems, 0.01)
...
func TestBloomFilterWithSize(t *testing.T) {
	...
	bf := bloomfilter.NewBloomFilterWithSize(bitSize, numHash)

	// Note: size and numHash are unexported, verify via stats instead
	stats := bf.GetStats()
	if stats.Size != bitSize {
		t.Errorf("Expected size %d, got %d", bitSize, stats.Size)
	}
	if stats.NumHashFunctions != numHash {
		t.Errorf("Expected %d hash functions, got %d", numHash, stats.NumHashFunctions)
	}
...
func BenchmarkBloomFilterAdd(b *testing.B) {
	bf := bloomfilter.NewBloomFilter(b.N, 0.01)
	b.ResetTimer()
