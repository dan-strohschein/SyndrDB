...
func TestTokenizer_SimpleSelect(t *testing.T) {
	...
	expected := []syndrQL.TokenType{
		syndrQL.TOKEN_SELECT,
		syndrQL.TOKEN_MULTIPLY,
		syndrQL.TOKEN_FROM,
		syndrQL.TOKEN_IDENT,
		syndrQL.TOKEN_WHERE,
		syndrQL.TOKEN_IDENT,
		syndrQL.TOKEN_GT,
		syndrQL.TOKEN_NUMBER,
		syndrQL.TOKEN_EOF,
	}
...
func TestTokenizer_InsertDocument(t *testing.T) {
	...
	_ = []syndrQL.TokenType{
		syndrQL.TOKEN_INSERT,
		syndrQL.TOKEN_DOCUMENT,
		syndrQL.TOKEN_INTO,
		syndrQL.TOKEN_IDENT,
		syndrQL.TOKEN_LBRACE,
		syndrQL.TOKEN_STRING,
		syndrQL.TOKEN_COLON,
		syndrQL.TOKEN_STRING,
		syndrQL.TOKEN_COMMA,
		syndrQL.TOKEN_STRING,
		syndrQL.TOKEN_COLON,
		syndrQL.TOKEN_NUMBER,
		syndrQL.TOKEN_RBRACE,
		syndrQL.TOKEN_EOF,
	}

	// Adjust for missing syndrQL.TOKEN_COLON in current implementation
	expectedAdjusted := []syndrQL.TokenType{
		syndrQL.TOKEN_INSERT,
		syndrQL.TOKEN_DOCUMENT,
		syndrQL.TOKEN_INTO,
		syndrQL.TOKEN_IDENT,
		syndrQL.TOKEN_LBRACE,
		syndrQL.TOKEN_STRING,
		syndrQL.TOKEN_ILLEGAL, // : not defined yet
		syndrQL.TOKEN_STRING,
		syndrQL.TOKEN_COMMA,
		syndrQL.TOKEN_STRING,
		syndrQL.TOKEN_ILLEGAL, // : not defined yet
	...
	// Verify key tokens
	if tokens[0].Type != syndrQL.TOKEN_INSERT {
...
func TestTokenizer_Operators(t *testing.T) {
	tests := []struct {
		...
		{"-", syndrQL.TOKEN_MINUS},
		{"*", syndrQL.TOKEN_MULTIPLY},
		{"/", syndrQL.TOKEN_DIVIDE},
		{"%", syndrQL.TOKEN_MODULO},
	}

	for _, tt := range tests {
		tokenizer := syndrQL.NewTokenizer(tt.input)
		tokens, err := tokenizer.Tokenize()
...
func TestTokenizer_Keywords(t *testing.T) {
	tests := []struct {
		...
		{"SELECT", syndrQL.TOKEN_SELECT},
		{"select", syndrQL.TOKEN_SELECT}, // Case insensitive
		{"INSERT", syndrQL.TOKEN_INSERT},
		{"UPDATE", syndrQL.TOKEN_UPDATE},
		{"DELETE", syndrQL.TOKEN_DELETE},
		{"FROM", syndrQL.TOKEN_FROM},
		{"WHERE", syndrQL.TOKEN_WHERE},
		{"AND", syndrQL.TOKEN_AND},
		{"OR", syndrQL.TOKEN_OR},
		{"NOT", syndrQL.TOKEN_NOT},
		{"TRUE", syndrQL.TOKEN_TRUE},
		{"FALSE", syndrQL.TOKEN_FALSE},
		{"NULL", syndrQL.TOKEN_NULL},
		{"LIKE", syndrQL.TOKEN_LIKE},
		{"IN", syndrQL.TOKEN_IN},
		{"CONTAINS", syndrQL.TOKEN_CONTAINS},
	}
...
func TestTokenizer_StringLiterals(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if tokens[0].Type != syndrQL.TOKEN_STRING {
			t.Errorf("Input '%s': expected syndrQL.TOKEN_STRING, got %s",
...
func TestTokenizer_Numbers(t *testing.T) {
	...
	for _, tt := range tests {
		...
		if tokens[0].Type != syndrQL.TOKEN_NUMBER {
			t.Errorf("Input '%s': expected syndrQL.TOKEN_NUMBER, got %s",
				tt.input, tokens[0].Type.String())
		}
...
func TestTokenizer_ComplexQuery(t *testing.T) {
	...
	if tokens[0].Type != syndrQL.TOKEN_SELECT {
		t.Errorf("Expected SELECT, got %s", tokens[0].Type.String())
	}

	if tokens[len(tokens)-1].Type != syndrQL.TOKEN_EOF {
	...
	for i, tok := range tokens {
		if tok.Type == syndrQL.TOKEN_LIMIT {
			if i+1 >= len(tokens) {
				t.Errorf("LIMIT token found but no number follows")
			} else if tokens[i+1].Type != syndrQL.TOKEN_NUMBER {
...
func TestTokenizer_ErrorHandling(t *testing.T) {
	...
	if len(tokens) > 0 && tokens[0].Type != syndrQL.TOKEN_ILLEGAL {
