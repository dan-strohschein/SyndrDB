...
func TestInternalNodeMerging(t *testing.T) {
	...
	stats, err := btreeindexV2.CalculateTreeStatistics(idx)
	require.NoError(t, err)
	...
	finalStats, err := btreeindexV2.CalculateTreeStatistics(idx)
	require.NoError(t, err)
...
func TestRootDemotion(t *testing.T) {
	...
	initialStats, err := btreeindexV2.CalculateTreeStatistics(idx)
	require.NoError(t, err)
...
func validateTreeStructure(t *testing.T, idx *btreeindexV2.BTreeIndex) error {
