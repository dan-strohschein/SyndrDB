...
func TestExpressionParser_NumberLiteral(t *testing.T) {
	...
	for _, tt := range tests {
		expr, err := syndrQL.ParseExpression(tt.input)
		if err != nil {
			t.Fatalf("Failed to parse '%s': %v", tt.input, err)
		}

		lit, ok := expr.(*syndrQL.LiteralExpression)
		if !ok {
...
func TestExpressionParser_NullLiteral(t *testing.T) {
	expr, err := syndrQL.ParseExpression("NULL")
	if err != nil {
...
func TestExpressionParser_Identifier(t *testing.T) {
	...
	for _, input := range tests {
		expr, err := syndrQL.ParseExpression(input)
		if err != nil {
			t.Fatalf("Failed to parse '%s': %v", input, err)
		}

		ident, ok := expr.(*syndrQL.IdentifierExpression)
		if !ok {
...
func TestExpressionParser_SimpleBinary(t *testing.T) {
	tests := []struct {
		input    string
		operator syndrQL.TokenType
	}{
		{"Age == 25", syndrQL.TOKEN_EQ},
		{"Age != 30", syndrQL.TOKEN_NEQ},
		{"Age > 18", syndrQL.TOKEN_GT},
		{"Age >= 21", syndrQL.TOKEN_GTE},
		{"Age < 65", syndrQL.TOKEN_LT},
		{"Age <= 100", syndrQL.TOKEN_LTE},
		{"x + y", syndrQL.TOKEN_PLUS},
		{"x - y", syndrQL.TOKEN_MINUS},
		{"x * y", syndrQL.TOKEN_MULTIPLY},
		{"x / y", syndrQL.TOKEN_DIVIDE},
		{"x % y", syndrQL.TOKEN_MODULO},
	}
...
func TestExpressionParser_LogicalOperators(t *testing.T) {
	tests := []struct {
		...
		{"Age > 18 AND Country == \"USA\"", syndrQL.TOKEN_AND},
		{"Age < 18 OR Country == \"Canada\"", syndrQL.TOKEN_OR},
	}
...
func TestExpressionParser_SpecialOperators(t *testing.T) {
	tests := []struct {
		...
		{"Name LIKE \"John%\"", syndrQL.TOKEN_LIKE},
		{"Age IN [18, 21, 25]", syndrQL.TOKEN_IN},
		{"Tags CONTAINS \"golang\"", syndrQL.TOKEN_CONTAINS},
	}
...
func TestExpressionParser_UnaryExpression(t *testing.T) {
	tests := []struct {
		...
		{"NOT active", syndrQL.TOKEN_NOT},
		{"-value", syndrQL.TOKEN_MINUS},
	}
	...
	for _, tt := range tests {
		...
		unary, ok := expr.(*syndrQL.UnaryExpression)
		if !ok {
...
func TestExpressionParser_ArrayExpression(t *testing.T) {
	...
	for _, tt := range tests {
		...
		arr, ok := expr.(*syndrQL.ArrayExpression)
		if !ok {
...
func TestExpressionParser_FunctionCall(t *testing.T) {
	...
	for _, tt := range tests {
		...
		call, ok := expr.(*syndrQL.CallExpression)
		if !ok {
...
func TestPatternRecognizer_Equality(t *testing.T) {
	expr, err := syndrQL.ParseExpression("Age == 25")
	if err != nil {
		t.Fatalf("Failed to parse: %v", err)
	}

	recognizer := syndrQL.NewPatternRecognizer()
...
func TestPatternRecognizer_Range(t *testing.T) {
	expr, err := syndrQL.ParseExpression("Age > 18")
	if err != nil {
		t.Fatalf("Failed to parse: %v", err)
	}
...
func TestPatternRecognizer_InList(t *testing.T) {
	expr, err := syndrQL.ParseExpression("Age IN [18, 21, 25]")
	if err != nil {
		t.Fatalf("Failed to parse: %v", err)
	}
...
func TestPatternRecognizer_Contains(t *testing.T) {
	expr, err := syndrQL.ParseExpression("Tags CONTAINS \"golang\"")
	if err != nil {
		t.Fatalf("Failed to parse: %v", err)
	}
...
func TestExpressionOptimizer_ConstantFolding(t *testing.T) {
	...
	optimizer := syndrQL.NewExpressionOptimizer()
	...
	for _, tt := range tests {
		...
		lit, ok := optimized.(*syndrQL.LiteralExpression)
		if !ok {
...
func TestExpressionOptimizer_DoubleNegation(t *testing.T) {
	expr, err := syndrQL.ParseExpression("NOT NOT active")
	if err != nil {
		t.Fatalf("Failed to parse: %v", err)
	}
	...
	ident, ok := optimized.(*syndrQL.IdentifierExpression)
	if !ok {
...
func TestExpressionValidator_ValidFunctions(t *testing.T) {
	...
	validator := syndrQL.NewExpressionValidator()
...
func TestExpressionValidator_InvalidFunction(t *testing.T) {
	expr, err := syndrQL.ParseExpression("UNKNOWN_FUNCTION()")
	if err != nil {
		t.Fatalf("Failed to parse: %v", err)
	}
...
func TestExpressionStringBuilder_Normalization(t *testing.T) {
	...
	builder := &syndrQL.ExpressionStringBuilder{}
...
func TestExpressionParser_InvalidSyntax(t *testing.T) {
	...
	for _, input := range tests {
		_, err := syndrQL.ParseExpression(input)
		if err == nil {
...
func BenchmarkPatternRecognizer_Recognize(b *testing.B) {
	expr, _ := syndrQL.ParseExpression("Age == 25")
	recognizer := syndrQL.NewPatternRecognizer()
...
func BenchmarkExpressionOptimizer_Optimize(b *testing.B) {
	expr, _ := syndrQL.ParseExpression("2 + 3 * 4 + 5")
	optimizer := syndrQL.NewExpressionOptimizer()
