...
// createTestDocumentsWithStrings creates test documents with string values.
// Documents use Data; GetFieldValueForSort in the sort package reads from Data when schema is passed.
func createTestDocumentsWithStrings(count int, fieldName string, prefix string) map[string]*models.Document {
	...
	for i := 0; i < count; i++ {
		doc := &models.Document{
			...
			Data:       map[string]interface{}{fieldName: fmt.Sprintf("%s%03d", prefix, i)},
		}
...
// getSortResultString returns the string value of fieldName from a sort result document (Values or Data).
func getSortResultString(doc *models.Document, schema *models.BundleFieldSchema, fieldName string) (string, bool) {
	if doc == nil {
		return "", false
	}
...
// getSortResultInterface returns the raw value of fieldName from a sort result document (for []byte etc.).
func getSortResultInterface(doc *models.Document, schema *models.BundleFieldSchema, fieldName string) (interface{}, bool) {
	if doc == nil {
		return nil, false
	}
	if schema != nil && len(doc.Values) > 0 {
...
func TestParallelStringSort_BasicCorrectness(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := ParallelStringSort(docs, "name", tt.ascending, true, tt.numWorkers, logger, schema)
			if err != nil {
			...
			for i := 0; i < len(result)-1; i++ {
				name1, _ := getSortResultString(result[i], schema, "name")
				name2, _ := getSortResultString(result[i+1], schema, "name")
...
func TestParallelStringSort_VariedStrings(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		doc := &models.Document{
			DocumentID: uuid.New().String(),
			Data:       map[string]interface{}{"text": fmt.Sprintf("%s_%03d", pattern, i)},
		}
		docs[doc.DocumentID] = doc
	}

	schema := sortSchema("text")
	result, err := ParallelStringSort(docs, "text", true, true, 4, logger, schema)
	if err != nil {
		t.Fatalf("ParallelStringSort failed: %v", err)
	}

	for i := 0; i < len(result)-1; i++ {
		text1, _ := getSortResultString(result[i], schema, "text")
		text2, _ := getSortResultString(result[i+1], schema, "text")
...
func TestParallelStringSort_WithSIMD(t *testing.T) {
	...
	resultWithSIMD, err := ParallelStringSort(docs, "description", true, true, 4, logger, schema)
	if err != nil {
		t.Fatalf("ParallelStringSort with SIMD failed: %v", err)
	}

	resultWithoutSIMD, err := ParallelStringSort(docs, "description", true, false, 4, logger, schema)
	if err != nil {
	...
	for i := 0; i < len(resultWithSIMD); i++ {
		val1, _ := getSortResultString(resultWithSIMD[i], schema, "description")
		val2, _ := getSortResultString(resultWithoutSIMD[i], schema, "description")
...
func TestParallelStringSort_EmptyInput(t *testing.T) {
	...
	result, err := ParallelStringSort(docs, "name", true, true, 4, logger, schema)
	if err != nil {
...
func TestParallelStringSort_ByteArrayValues(t *testing.T) {
	logger := zap.NewNop().Sugar()
	schema := sortSchema("data")

	docs := make(map[string]*models.Document)
	for i := 0; i < 100; i++ {
		doc := &models.Document{
			DocumentID: uuid.New().String(),
			Data:       map[string]interface{}{"data": []byte(fmt.Sprintf("bytes_%03d", i))},
		}
		docs[doc.DocumentID] = doc
	}

	result, err := ParallelStringSort(docs, "data", true, true, 4, logger, schema)
	if err != nil {
		t.Fatalf("ParallelStringSort failed: %v", err)
	}
	...
	for i := 0; i < len(result)-1; i++ {
		v1, _ := getSortResultInterface(result[i], schema, "data")
		v2, _ := getSortResultInterface(result[i+1], schema, "data")
		bytes1, ok1 := v1.([]byte)
		bytes2, ok2 := v2.([]byte)
		if !ok1 || !ok2 {
			t.Errorf("At index %d: expected []byte", i)
			continue
		}
		str1, str2 := string(bytes1), string(bytes2)
		if str1 > str2 {
...
func BenchmarkParallelStringSort(b *testing.B) {
	...
	for _, size := range sizes {
		...
		b.Run(fmt.Sprintf("size=%d", size), func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, _ = ParallelStringSort(docs, "name", true, true, runtime.NumCPU(), logger, schema)
			}
		})
	}
}
...
func BenchmarkParallelStringSort_Workers(b *testing.B) {
	...
	for _, numWorkers := range workers {
		b.Run(fmt.Sprintf("workers=%d", numWorkers), func(b *testing.B) {
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				_, _ = ParallelStringSort(docs, "name", true, true, numWorkers, logger, schema)
			}
		})
	}
}
...
func BenchmarkParallelStringSort_SIMD(b *testing.B) {
	...
	b.Run("WithSIMD", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _ = ParallelStringSort(docs, "description", true, true, runtime.NumCPU(), logger, schema)
		}
	})
	...
	b.Run("WithoutSIMD", func(b *testing.B) {
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			_, _ = ParallelStringSort(docs, "description", true, false, runtime.NumCPU(), logger, schema)
		}
	})
}
