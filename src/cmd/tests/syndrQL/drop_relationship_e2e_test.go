...
func TestDropRelationship_MultipleRelationships(t *testing.T) {
	...
	createRelationship1Cmd := `UPDATE BUNDLE "Authors" ADD RELATIONSHIP ("1toMany", "Authors", "DocumentID", "Books", "AuthorsID");`
	startTime = time.Now()
	...
	createRelationship2Cmd := `UPDATE BUNDLE "Authors" ADD RELATIONSHIP ("1toMany", "Authors", "DocumentID", "Publishers", "AuthorsID");`
	startTime = time.Now()
