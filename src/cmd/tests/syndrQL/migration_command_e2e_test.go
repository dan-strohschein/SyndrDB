...
func TestMigrationCommand_CreateBundleRollback(t *testing.T) {
	...
	// Note: May also fail with lock-related errors if lock wasn't released from previous operation
	if err == nil {
		t.Error("Expected rollback to fail (no down commands), but it succeeded")
	} else if !strings.Contains(err.Error(), "no down commands") &&
		!strings.Contains(err.Error(), "migration already in progress") &&
		!strings.Contains(err.Error(), "failed to acquire lock") {
		t.Errorf("Expected 'no down commands', 'migration already in progress', or 'failed to acquire lock' error, got: %v", err)
	} else {
