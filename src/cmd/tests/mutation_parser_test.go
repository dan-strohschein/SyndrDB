...
// DISABLED: This file partially tests internal unexported methods
//
// This test file includes tests for:
//   - parser.resolveArgumentValue() - unexported method (lines 400-549, ~150 lines of tests)
//   - parser.logger - unexported field
//
// The file contains both public API tests (which would work) and internal method tests.
//
// To enable this test:
//   1. Either move entire file to src/internal/graphQL/mutations/mutation_parser_test.go
//      (tests in the same package can access unexported methods)
//      and change package declaration from "package main" to "package mutations"
//   2. Or split the file: keep public API tests here, move internal tests to mutations package
//   3. Remove the `// +build ignore` line above
//
// Alternatively, export resolveArgumentValue if it needs to be tested from external packages.
...
type MutationParser = mutations.MutationParser

var NewMutationParser = mutations.NewMutationParser
...
func TestNewMutationParser(t *testing.T) {
	...
	// Note: parser.logger is unexported and cannot be tested directly
}
