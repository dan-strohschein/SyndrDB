...
func LoadBundleIntoMemory(database *Database, bundleID string) (*[]byte, *Bundle, error) {
	bundleFile, err := OpenDataFile(database, bundleID)
	if err != nil {
		return nil, nil, fmt.Errorf("error opening bundle file %s: %w", bundleID, err)
	}
	...
	bundleData, err := DecodeDocumentBSON(data)
	if err != nil {
...
func OpenDataFile(database *Database, fileName string) (*os.File, error) {
	// Open a specific data file
	filePath := filepath.Join(database.DataFilePath, fileName)
...
func CreateBundleFile(database *Database, bundle *Bundle) (*os.File, error) {
	// Create a new data file
	filePath := filepath.Join(database.DataFilePath, bundle.BundleID)

	// Check if the file already exists
	if helpers.FileExists(filePath) {
		return nil, fmt.Errorf("Bundle %s already exists", bundle.BundleID)
	}

	file, err := os.Create(filePath)
	if err != nil {
		return nil, fmt.Errorf("error creating data file %s: %w", bundle.BundleID, err)
	}
	...
	encodedBundle, err := EncodeDocumentBSON(convertedBundle)
	if err != nil {
		return nil, fmt.Errorf("error encoding bundle data: %w", err)
	}

	// Write the encoded bundle to the file
	fileLen, err := file.Write(encodedBundle)
	if err != nil {
		return nil, fmt.Errorf("error writing to bundle data file %s: %w", bundle.BundleID, err)
	}

	if fileLen != len(encodedBundle) {
		return nil, fmt.Errorf("error writing to bundle data file %s: wrote %d bytes, expected %d", bundle.BundleID, fileLen, len(encodedBundle))
	}
...
func EncodeDocumentBSON(jsonData map[string]interface{}) ([]byte, error) {
...
func DecodeDocumentBSON(bsonData []byte) (interface{}, error) {
