...
// This file contains unit tests for the ComplexityAnalyzer to ensure
// correct depth, breadth, and cost calculations.
...
func TestShouldUseDataLoader(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := ShouldUseDataLoader(tt.result)
...
func TestShouldUseJoins(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := &ComplexityResult{Depth: tt.depth}
			got := ShouldUseJoins(result)
			if got != tt.expected {
