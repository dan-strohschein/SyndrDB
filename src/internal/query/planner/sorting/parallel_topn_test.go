...
// TestDivideDocumentsIntoChunks verifies chunk division logic
func TestDivideDocumentsIntoChunks(t *testing.T) {
	tests := []struct {
		name      string
		numDocs   int
		numChunks int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			chunks := divideDocumentsIntoChunks(docs, tt.numChunks)
