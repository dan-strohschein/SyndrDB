...
// TestParallelStringSort_CompareWithSequential verifies parallel matches sequential results
func TestParallelStringSort_CompareWithSequential(t *testing.T) {
	logger := zap.NewNop().Sugar()
	...
	for _, tt := range tests {
		t.Run(fmt.Sprintf("%d docs", tt.numDocs), func(t *testing.T) {
			docs := createTestDocumentsWithStrings(tt.numDocs, "title", "item")

			// Run sequential sort using StringHeapSort with large limit
			// (This sorts all docs, effectively doing a full sort)
			seqKeys := make([]AbbreviatedKey, 0, len(docs))
			for _, doc := range docs {
				field := doc.Fields["title"]
				strVal := field.Value.(string)
				key := generateAbbreviatedKey(strVal, doc)
				seqKeys = append(seqKeys, key)
			}
			sortAbbreviatedKeys(seqKeys, true, true)
			seqResult := make([]*models.Document, len(seqKeys))
			for i, key := range seqKeys {
				seqResult[i] = key.Document
			}

			// Run parallel sort
			parResult, err := ParallelStringSort(docs, "title", true, true, 4, logger)
			if err != nil {
				t.Fatalf("ParallelStringSort failed: %v", err)
			}
			...
			for i := 0; i < len(seqResult); i++ {
				seqVal := seqResult[i].Fields["title"].Value.(string)
				parVal := parResult[i].Fields["title"].Value.(string)

				if seqVal != parVal {
					t.Errorf("Value mismatch at index %d: sequential=%s, parallel=%s",
						i, seqVal, parVal)
				}
			}
		})
