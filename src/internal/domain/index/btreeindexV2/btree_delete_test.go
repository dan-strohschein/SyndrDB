...
func TestDelete_CreatesTombstone(t *testing.T) {
	...
	assert.Equal(t, uint64(0), idx.metadata.TotalTombstones, "Should have 0 tombstones initially")
	assert.Equal(t, 0.0, idx.metadata.TombstoneRatio, "Tombstone ratio should be 0")
	...
	assert.Equal(t, uint64(1), idx.metadata.TotalTombstones, "Should have 1 tombstone after deletion")
	assert.Greater(t, idx.metadata.TombstoneRatio, 0.0, "Tombstone ratio should be > 0")

	t.Logf("Tombstone created successfully: TotalTombstones=%d, TombstoneRatio=%.4f",
		idx.metadata.TotalTombstones, idx.metadata.TombstoneRatio)
}
...
func TestDelete_UpdatesTombstoneRatio(t *testing.T) {
	...
	assert.Equal(t, uint64(deleteCount), idx.metadata.TotalTombstones, "Should have %d tombstones", deleteCount)
	...
	t.Logf("Tombstone tracking verified: TotalTombstones=%d, TotalRecords=%d, Ratio=%.4f",
		idx.metadata.TotalTombstones, idx.metadata.TotalRecords, idx.metadata.TombstoneRatio)
}
...
func TestDelete_CompactionNeededAt20Percent(t *testing.T) {
	...
	t.Logf("Initial state: TotalRecords=%d, TotalTombstones=%d",
		idx.metadata.TotalRecords, idx.metadata.TotalTombstones)
	...
	// Verify tombstone count
	assert.Equal(t, uint64(4), idx.metadata.TotalTombstones,
	...
	assert.Equal(t, uint64(6), idx.metadata.TotalTombstones,
		"Should have 6 tombstones after 6 deletions")
	assert.Greater(t, idx.metadata.TombstoneRatio, 0.20,
	...
	t.Logf("After 6 deletions: TotalTombstones=%d, TotalRecords=%d, Ratio=%.4f, CompactionNeeded=%v",
		idx.metadata.TotalTombstones, idx.metadata.TotalRecords, idx.metadata.TombstoneRatio, idx.metadata.CompactionNeeded)
}
...
func TestDelete_NodeLevelFragmentation(t *testing.T) {
	...
	assert.Greater(t, idx.metadata.NodesNeedCompaction, uint32(0),
	...
	t.Logf("Node fragmentation tracked: NodesNeedCompaction=%d, TotalTombstones=%d",
		idx.metadata.NodesNeedCompaction, idx.metadata.TotalTombstones)
}
...
func TestDelete_MultipleDeletionsSameKey(t *testing.T) {
	...
	assert.Equal(t, uint64(deleteCount), idx.metadata.TotalTombstones,
	...
	assert.True(t, idx.metadata.CompactionNeeded,
...
func TestDelete_NonExistentKey(t *testing.T) {
	...
	assert.Equal(t, uint64(0), idx.metadata.TotalTombstones,
...
func TestDelete_TombstoneAccumulation(t *testing.T) {
	...
	for batchNum, deleteCount := range batches {
		...
		// Verify accumulation
		assert.Equal(t, uint64(totalDeleted), idx.metadata.TotalTombstones,
		...
		t.Logf("After batch %d: TotalTombstones=%d, TotalRecords=%d, Ratio=%.4f",
			batchNum, idx.metadata.TotalTombstones, idx.metadata.TotalRecords, idx.metadata.TombstoneRatio)
	}
	...
	t.Logf("Final state: TotalTombstones=%d, TotalRecords=%d",
		idx.metadata.TotalTombstones, idx.metadata.TotalRecords)
}
...
func setupTestIndex(t *testing.T, testName string) (string, *BTreeIndex) {
	...
	// Create index
	idx, err := CreateBTreeIndex(config, sugaredLogger)
	require.NoError(t, err)
