...
func TestCacheEviction_LRUOrder(t *testing.T) {
	...
	// Get initial stats
	stats := idx.pageManager.GetPageManagerCacheStats()
...
func TestCacheEviction_DirtyPageFlushing(t *testing.T) {
	...
	t.Logf("=== TREE STRUCTURE AFTER ALL INSERTS ===")
	rootPage, err := idx.pageManager.GetPage(idx.metadata.RootPageNum, func(pn uint32) (interface{}, error) {
		return idx.fileManager.ReadPage(pn)
	})
	require.NoError(t, err)
	root, ok := rootPage.(*BTreeNode)
	require.True(t, ok)
	t.Logf("Root page %d: IsLeaf=%v, KeyCount=%d", idx.metadata.RootPageNum, root.IsLeaf, root.KeyCount)
	for i := uint32(0); i < root.KeyCount; i++ {
		t.Logf("  Key[%d] = %s", i, string(root.Keys[i]))
	}
	if !root.IsLeaf {
		t.Logf("  Children: %v", root.Children[:root.KeyCount+1])
	}
...
func TestCacheEviction_HitMissStatistics(t *testing.T) {
	...
	// Get initial stats
	statsAfterInsert := idx.pageManager.GetPageManagerCacheStats()
	...
	statsAfterSearch := idx.pageManager.GetPageManagerCacheStats()
