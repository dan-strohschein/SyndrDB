...
and limiting memory usage during query execution. It uses a 1-in-100 sampling strategy to
...
- Thread-Safe: Uses sync.Mutex for concurrent access during parallel operations
- Sampling-Based: Tracks every 100th document to reduce overhead to <1%
...
Each node calls Sample() every 100 documents, and the tracker maintains a running
average to project total memory usage.

SAMPLING STRATEGY:
- Tracks every 100th document (configurable via SAMPLE_RATE constant)
- TODO: I could make the sampling rate configurable via server settings
...
FUTURE ENHANCEMENTS:
- TODO: I could add support for admin users to override this limit via a query-level flag
- TODO: I could implement dynamic limit adjustment based on available system memory
- TODO: I could add adaptive sampling (1-in-10 for <1000 docs, 1-in-100 for >1000)
*/
...
type MemoryTracker struct {
	limit              int64      // Maximum allowed memory in bytes
	totalSampledBytes  int64      // Sum of all sampled document sizes
	sampleCount        int64      // Number of documents sampled
	documentsProcessed int64      // Total documents processed so far
	exceeded           bool       // Whether limit has been exceeded
...
func (mt *MemoryTracker) Sample(docSizeBytes int64, currentIndex int) error {
	...
	// Record this sample
	mt.totalSampledBytes += docSizeBytes
	mt.sampleCount++
...
func (mt *MemoryTracker) ProjectTotalMemory(totalExpected int) int64 {
	...
	avgDocSize := mt.totalSampledBytes / mt.sampleCount
...
func (mt *MemoryTracker) GetUsage(totalExpected int) (int64, int64, bool) {
	...
	return projected, mt.limit, mt.exceeded
}
...
func (mt *MemoryTracker) GetSampleStats() (int64, int64, int64) {
	...
	return mt.sampleCount, mt.documentsProcessed, avgDocSize
}
...
func (mt *MemoryTracker) RecordMetrics(totalExpected int) {
	...
	GetGlobalMemoryMetrics().RecordQuery(projected, mt.exceeded)
