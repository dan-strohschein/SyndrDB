...
type BTreeFile struct {
	...
	path         string
	file         *os.File
	metaPage     *buffer.BTreePage
	rootPageNum  uint32
	height       uint16
	pageCache    map[uint32]*buffer.BTreePage
	cacheSize    int
	maxCacheSize int
...
func OpenBTreeFile(path string, cacheSize int) (*BTreeFile, error) {
	...
	btree := &BTreeFile{
		path:         path,
		file:         file,
		pageCache:    make(map[uint32]*buffer.BTreePage),
		cacheSize:    0,
...
func (bt *BTreeFile) Find(key []byte) (*buffer.IndexTuple, error) {
	bt.RLock()
	defer bt.RUnlock()
...
func (bt *BTreeFile) FindRange(startKey, endKey []byte) ([]*buffer.IndexTuple, error) {
	bt.RLock()
	defer bt.RUnlock()

	var results []*buffer.IndexTuple
	...
	for !done {
		for _, entry := range currentPage.Entries {
			...
			if startKey == nil || bytes.Compare(entry.Key, startKey) >= 0 {
				...
				results = append(results, &buffer.IndexTuple{
					Key:   entry.Key,
					DocID: entry.DocID,
					TID:   tid,
				})
...
func (bt *BTreeFile) findLeafPage(key []byte) (*buffer.BTreePage, error) {
	// Start from the root
	pageNum := bt.rootPageNum
...
func (bt *BTreeFile) readPage(pageNum uint32) (*buffer.BTreePage, error) {
...
func (bt *BTreeFile) writePage(pageNum uint32, page *buffer.BTreePage) error {
	// Serialize the page
...
func (bt *BTreeFile) addToCache(pageNum uint32, page *buffer.BTreePage) {
...
func (bt *BTreeFile) writePageToDisk(pageNum uint32, page *buffer.BTreePage) error {
	// Serialize the page
	pageData, err := serializePage(page)
	if err != nil {
		return fmt.Errorf("failed to serialize page: %w", err)
...
func parsePage(data []byte) (*buffer.BTreePage, error) {
	if len(data) < 24 {
	...
	page := &buffer.BTreePage{
		...
		FreeSpace:  freeSpace,
		Entries:    make([]buffer.BTreeEntry, 0, numEntries),
	}
...
func serializePage(page *buffer.BTreePage) ([]byte, error) {
